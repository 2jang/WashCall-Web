// js/server-api.js
// ❗️ (api.reserveRoom 함수가 추가된 최종 버전)

// const API_BASE_URL = 'http://127.0.0.1:8000'; // 개발 환경
const API_BASE_URL = 'https://server.washcall.space'; // ❗️ [수정] washcall.space로 변경
...
const api = {
   ...
   // 4. 사용자 로그인 (POST)
    login: async function(studentId, password) {
        ...
        // ❗️ (서버 팀이 fcm_token을 Optional로 바꿨다고 가정)
        const payload = {
            user_snum: parseInt(studentId, 10), 
            user_password: password,            
            fcm_token: "TEMP_TOKEN_ON_LOGIN" // (서버 수정 전까지 임시방편 유지)
        };
    ...
    // 15. ❗️ [삭제] '개별 토글' 함수 (더 이상 사용 안 함)
    // toggleNotifyMe: async function(machineId, subscribe) { ... }

    // (구독 관련 함수는 push.js가 사용할 /reserve가 필요함)

    /**
     * ❗️ [신규] 17. '세탁실 알림 구독' (POST /reserve)
     * (push.js가 호출할 함수)
     */
    reserveRoom: async function(roomId, isReservedInt) {
        console.log(`API: ${roomId}번 세탁실 구독 요청 (요청값: ${isReservedInt})`);
        
        // web_schemas.py의 ReserveRequest 형식에 맞춤
        const payload = {
            room_id: parseInt(roomId, 10),
            isreserved: isReservedInt // ❗️ (1을 보낼 것임)
        };
